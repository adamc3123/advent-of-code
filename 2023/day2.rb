...
#
def sum_of_possible_games(str)
  puts str
  puts "---------------"
  possible_games = []
  str.downcase.split("\n").each_with_index do |game, index|
    invalid_game = false
    game.scan(/((\d+) (blue|red|green))/).each do |match_arr|
      #["3 blue", "3", "blue"]
      invalid_game = true if match_arr[1].to_i > COLOR_MAX[match_arr[2]]
    end
    possible_games << index + 1 unless invalid_game
  end
  possible_games.sum
end
...
# expect 8
#puts sum_of_possible_games(TEST)
#puts sum_of_possible_games(GAMES)
...
def power_of_min_sets(str)
  ...
  str.downcase.split("\n").each_with_index do |game, index|
    ...
    game.scan(/((\d+) (blue|red|green))/).each do |match_arr|
      ...
      case match_arr[2]
      when "red"
        minimums[0] = [match_arr[1].to_i, minimums[0]].max
      when "green"
        minimums[1] = [match_arr[1].to_i, minimums[1]].max
      when "blue"
        minimums[2] = [match_arr[1].to_i, minimums[2]].max
      else
        raise "BAD"
      end
    end
    #puts minimums.to_s + ": " + minimums.reject(&:zero?).inject(:*).to_s
    game_power << minimums.reject(&:zero?).inject(:*)
  end
  game_power.sum
end
...
# expect 2286
#puts power_of_min_sets(TEST)

puts power_of_min_sets(GAMES)
