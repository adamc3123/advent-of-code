...
def part1(str)
  calibration_value = str.split("\n").map do |code|
    first,second = nil
    code.each_char do |char|
      if char =~ /\d/
        first = char unless first
        second = char
      end
    end
    first.concat(second).to_i
  end
...
def to_num(str)
  NUMBERS.keys.include?(str) ? NUMBERS[str] : str
end

# 54868 is too low
def part2(str)
  calibration_value = str.split("\n").map do |code|
    result = code.match(/one|two|three|four|five|six|seven|eight|nine|1|2|3|4|5|6|7|8|9/)
    result_reverse = code.reverse.match(/eno|owt|eerht|ruof|evif|xis|neves|thgie|enin|1|2|3|4|5|6|7|8|9/)
    first = to_num(result.to_s)
    second = to_num(result_reverse.to_s.reverse)
    final = first + second
    #puts "code #{code}:"
    #puts "    first: #{first}"
    #puts "    second: #{second}"
...
#puts part1(CODES)
puts part2(CODES)
#puts part2(TEST_CODE)
#puts part2("xtwone3four")
